...
class CouponController {
  ...
  async store({ request, response, transform }) {
     ...
     * 2 - clients - pode ser utilizado apenas em clientes especificos
     * 3 - clients e products - pode ser utlizado somente em produtos e clientes especificos
     * 4 - pode ser utilziado por qualquer cliente em qualquer pedido
     */
    ...
    try {
      ...
      // Start service layer
      const service = new Service(couponData, trx)
  ...
  async update({ params: { id }, request, response }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({
        message: 'Não foi possivel atualizar esse cupom no momento!',
      })
    }
  }
  ...
  async destroy({ params: { id }, request, response }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({
        message: 'Não foi possivel deletar esse cupon!',
      })
    }
  }
}
