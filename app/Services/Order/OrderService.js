...
class OrderService {
  ...
  async canApplyDiscount(coupon) {
    ...
    if (
      ...
      // verifica se o cupom ja entou em validade
      // verifica se há uma data de expiração
      // se houver a data de expiraçãio, verifica se o cupom expirou
      return false
    }
    ...
    // Verificar se o cupom esta associado a produtos e clientes especificos
    if (
     ...
     */
    if (isAssociatedToClients && isAssociatedToProduts) {
    ...
    /**
     * Caso de uso 2 - O cupom esta associado apenas a produto
     */
    if (
      isAssociatedToProduts &&
      Array.isArray(productsMatch) &&
      productsMatch.length > 0
    ) {
      return true
    ...
    /**
     * Caso de uso 3 - O cupom esta associado a 1 ou mais clientes (e nenhum produto)
     */
    if (
    ...
    /**
     * Caso nenhuma das verificações acima deem possitivas
     * então o cupom esta associado a clientes ou produtos ou os dois
     * porém nenhum dos produtos deste pedido esta elegivel ao desconto
     * e o cliente que fez a compra também não poderá utilizar esse desconto
     */
    return false
  }
}
